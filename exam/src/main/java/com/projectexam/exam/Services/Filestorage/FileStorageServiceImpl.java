...
public class FileStorageServiceImpl implements FileStorageService {
    ...
    public FileStorageServiceImpl(FileStorageProperties properties) {
        String configuredPath = properties != null ? properties.getPath() : null;
        if (configuredPath == null || configuredPath.isBlank()) {
            // Fallback par défaut si aucune propriété n'est définie
            String userDir = System.getProperty("user.dir");
            configuredPath = userDir + "/uploads";
            LOGGER.warn("Aucune propriété 'filestorage.path' définie. Utilisation du dossier par défaut: {}", configuredPath);
        }
        this.rootLocation = Paths.get(configuredPath);
    }
